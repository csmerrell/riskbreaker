...
import {
    CompositeActor,
    isCompositeActor,
} from '@/game/actors/CompositeActor/CompositeActor';
...
import { PartyMenuManager } from '../exploration/PartyMenuManager';
...
export class HeadshotManager {
    ...
    constructor(private parent: BattleManager | PartyMenuManager) {}
    ...
    async captureTemporalSnapshot(
        ...
        // Create portrait canvas at display resolution
        const portraitCanvas = document.createElement('canvas');
        portraitCanvas.width = spriteWidth * Math.abs((opts.scale?.x ?? 1) * getScale());
        portraitCanvas.height = spriteHeight * Math.abs((opts.scale?.y ?? 1) * getScale());
        const portraitCtx = portraitCanvas.getContext('2d')!;
        portraitCtx.imageSmoothingEnabled = false;

        const captureAnchor = vec(0, 0);
        ...
        portraitCtx.drawImage(
            ...
            captureAnchor.x, // source x
            captureAnchor.y, // source y
            spriteHeight * pixelRatio, // source width
            spriteWidth * pixelRatio, // source height
            0, // dest x
            0, // dest y
            portraitCanvas.width, // dest width
            portraitCanvas.height, // dest height
